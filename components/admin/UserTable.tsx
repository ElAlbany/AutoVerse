        ...
        {optimisticUsers.map((user) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-dark-muted">
              <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 dark:bg-dark-surface text-gray-700 dark:text-dark-text font-bold transition-colors duration-500">
                {user._count.orders}
              </span>
              {user._count.orders === 1 ? "order" : "orders"} placed
            </div>

            <button
              onClick={() => handleDelete(user)}
              disabled={user.id === currentUserId || deletingId === user.id}
              className={`w-full text-center py-2 rounded-xl text-sm font-medium transition-colors ${
                user.id === currentUserId
                  ? "text-gray-300 dark:text-dark-border cursor-not-allowed bg-gray-50 dark:bg-white/5"
